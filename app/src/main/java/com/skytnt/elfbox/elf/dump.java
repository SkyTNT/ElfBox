...
public class dump
{
	...
	public int getSymNum(section sec){
		return sec.size/16;
	}
	
	public symbol getSym(section sec,int index){
	...
	public int getRelNum(section sec){
		return sec.size/sec.entsize;
	}
	
	public Vector<relocation> getRels(){
		Vector<relocation> rels=new Vector<relocation>();		
		for(section sec:elf.sections){
			if(sec.type==9){
	...
	public relocation getRel(section sec,int index){
	...
	public symbol Rel2Sym(relocation rel){
		for(section sec:elf.sections){
			if(sec.type==11){
	...
	public String getString(section strtb,int off){
